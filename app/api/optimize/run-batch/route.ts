...
async function performOptimization(body: any) {
  ...
  for (let nodes = Math.max(1, body?.scenario?.minNodes || 1); nodes <= Math.max(1, body?.scenario?.maxNodes || 5); nodes += Math.max(1, body?.scenario?.step || 1)) {
    ...
    try {
      ...
      const inventoryCostAllYears = inventoryResult.total_cost * defaultForecast.length;
...
export async function POST(req: NextRequest) {
  try {
    ...
    // If a scenario_id is provided, create an async job and return job id immediately
    if (body?.scenario_id) {
      const scenarioId = Number(body.scenario_id);
      ...
      (async () => {
        ...
        try {
          ...
          // Merge scenario.metadata or payload into body for optimization params
          const merged = { ...body, scenario: { ...(body.scenario || {}), ...(scenario.metadata || {}), ...(scenario || {}) } };
    ...
    // Otherwise, run synchronously (legacy behavior)
    const response = await performOptimization(body);
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, error: err?.message || 'job status error' }, { status: 500 });
  }
}
